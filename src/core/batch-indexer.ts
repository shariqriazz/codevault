...
export class BatchEmbeddingProcessor {
  ...
  /**
   * Process the current batch of chunks with smart error handling
   */
  private async processBatch(retryCount = 0): Promise<void> {
    ...
    try {
      ...
      if (isBatchSizeError(error) && currentBatch.length > 1) {
        ...
        console.log(`⚠️  Batch size too large (${currentBatch.length} chunks), splitting and retrying...`);
        ...
        this.processing = false; // Release lock before recursion
        
        // Process both halves
        this.batch = firstHalf;
        await this.processBatch(0);
        this.batch = secondHalf;
        await this.processBatch(0);
        return;
      } else if (isRateLimitError(error) && retryCount < MAX_BATCH_RETRIES) {
        // Rate limit error - exponential backoff
        const delay = INITIAL_RETRY_DELAY_MS * Math.pow(2, retryCount);
        console.log(`⚠️  Rate limit hit, retrying batch in ${delay}ms (attempt ${retryCount + 1}/${MAX_BATCH_RETRIES})...`);
        await new Promise(resolve => setTimeout(resolve, delay));
        
        this.processing = false; // Release lock before retry
        this.batch = currentBatch;
        await this.processBatch(retryCount + 1);
        return;
      }
  ...
  private async fallbackToIndividualProcessing(batch: ChunkToEmbed[]): Promise<void> {
    ...
    if (errors.length > 0) {
      console.warn(`⚠️  ${errors.length}/${batch.length} chunks failed in fallback processing`);
